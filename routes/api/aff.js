    ...
    try {
      if (affirmationFields.affirmationType === 'gratitude') {
        ...
        const user = await DB.User.findOneAndUpdate(
          ...
          { $push: { 'affirmations.gratitudes': gratitude._id } },
          { new: true }
        );
        return res.json({ gratitude, user });
      } else if (affirmationFields.affirmationType === 'quote') {
        const quote = new DB.Quote(affirmationFields);
        await quote.save();
        const user = await DB.User.findOneAndUpdate(
          { _id: affirmationFields.user },
          { $push: { 'affirmations.quotes': quote._id } },
          { new: true }
        );
        return res.json({ quote, user });
      } else {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const user = await DB.User.findOneAndUpdate(
        { _id: affirmationFields.user },
        { $push: { 'affirmations.achievements': achievement._id } },
        { new: true }
      );
      return res.json({ achievement, user });
    } catch (err) {
      console.error(err.message);
      res.status(500).send('Server Error');
    }
